...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($driver === 'mysql') {
            ...
            DB::statement('ALTER TABLE avaliacoes ADD CONSTRAINT avaliacoes_nota_0_5_check CHECK (nota BETWEEN 0 AND 5)');
            return;
        }

        if ($driver === 'pgsql') {
            DB::statement('ALTER TABLE avaliacoes DROP CONSTRAINT IF EXISTS avaliacoes_nota_check');
            DB::statement('ALTER TABLE avaliacoes ADD CONSTRAINT avaliacoes_nota_check CHECK (nota BETWEEN 0 AND 5)');
        }
    }
