    ...
    <section class="panel-card">
        @forelse($avaliacoes as $avaliacao)
            <article class="support-item">
                <div>
                    <span class="status-pill status-publicado">{{ $avaliacao->nota }}/10</span>
                    ...
                    @else
                        <form action="{{ route('avaliacoes.responder', $avaliacao) }}" method="POST" class="mt-3">
                            @csrf
                            <textarea name="resposta_instrutor" class="form-control mb-2" rows="3" placeholder="Escreva uma resposta ao aluno..." required></textarea>
                            <button class="btn btn-primary" type="submit"><i class="bi bi-send me-2"></i>Responder</button>
                        </form>
                    @endif
                </div>
            </article>
